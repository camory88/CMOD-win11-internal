...
template <typename T>
inline bool SimpleIsValidAddress(T* address, int offsetted = 0) {
...
template <typename T>
inline bool IsValidAddress(T* address, std::string name, bool isImportant, int offsetted = 0, bool isPattern = true) {
...
#define RELINK(x, real)         \
	...
	static std::vector<UNLINKED_MODULE> m_unlinkedModules;
};